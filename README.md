# libgbm
libgbm is the Graphics Buffer Manager library, a frontend to GBM which various
vendors rely on to provide a GBM backend. In the case of Mesa, this will be
`dri_gbm`, and in the case of the NVIDIA driver, it will be `nvidia-drm_gbm`, a
symlink pointing to `libnvidia-allocator`.

It was and still is a part of the Mesa project. However, the `meson.build` file
...
makes sense to have it in its own package.
